...
func NewHTMLElementFromSelectionNode(url *url.URL, s *goquery.Selection, n *html.Node, idx int) *HTMLElement {
	return &HTMLElement{
		Name:       n.Data,
                URL:        url,
...
func (h *HTMLElement) ForEach(goquerySelector string, callback func(int, *HTMLElement)) {
	...
	h.DOM.Find(goquerySelector).Each(func(_ int, s *goquery.Selection) {
		for _, n := range s.Nodes {
			callback(i, NewHTMLElementFromSelectionNode(h.URL, s, n, i))
			i++
		}
	})
}
...
func (h *HTMLElement) ForEachWithBreak(goquerySelector string, callback func(int, *HTMLElement) bool) {
	...
	h.DOM.Find(goquerySelector).EachWithBreak(func(_ int, s *goquery.Selection) bool {
		for _, n := range s.Nodes {
			if callback(i, NewHTMLElementFromSelectionNode(h.URL, s, n, i)) {
				i++
				return true
			}
		}
