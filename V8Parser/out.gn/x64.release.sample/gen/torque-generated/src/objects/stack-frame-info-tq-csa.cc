...
TNode<Smi> LoadStackFrameInfoWasmFunctionIndex_0(compiler::CodeAssemblerState* state_, TNode<StackFrameInfo> p_o) {
  ...
  if (block0.is_used()) {
    TNode<StackFrameInfo> tmp0;
    ca_.Bind(&block0, &tmp0);
    TNode<IntPtrT> tmp1 = ca_.IntPtrConstant(StackFrameInfo::kWasmFunctionIndexOffset);
    USE(tmp1);
    TNode<Smi>tmp2 = CodeStubAssembler(state_).LoadReference<Smi>(CodeStubAssembler::Reference{tmp0, tmp1});
    ca_.Goto(&block2, tmp0, tmp2);
  }
...
void StoreStackFrameInfoWasmFunctionIndex_0(compiler::CodeAssemblerState* state_, TNode<StackFrameInfo> p_o, TNode<Smi> p_v) {
  ...
  if (block0.is_used()) {
    ...
    TNode<Smi> tmp1;
    ca_.Bind(&block0, &tmp0, &tmp1);
    TNode<IntPtrT> tmp2 = ca_.IntPtrConstant(StackFrameInfo::kWasmFunctionIndexOffset);
    USE(tmp2);
    CodeStubAssembler(state_).StoreReference(CodeStubAssembler::Reference{tmp0, tmp2}, tmp1);
    ca_.Goto(&block2, tmp0, tmp1);
  }
